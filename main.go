...
func mainE(ctx context.Context) error {
	...
	daemonCommand.PersistentFlags().Bool(f.Service.Installation.Guest.Kubernetes.API.Security.PrivateWhitelist.Enabled, false, "Enable or disable guest cluster k8s private API whitelisting.")
	daemonCommand.PersistentFlags().String(f.Service.Installation.Guest.Kubernetes.API.Security.PrivateWhitelist.SubnetList, "", "Subnet list for guest cluster k8s private API whitelisting.")
	daemonCommand.PersistentFlags().Bool(f.Service.Installation.Guest.Kubernetes.API.Security.Whitelist.Enabled, false, "Enable or disable guest cluster k8s public API whitelisting.")
	daemonCommand.PersistentFlags().String(f.Service.Installation.Guest.Kubernetes.API.Security.Whitelist.SubnetList, "", "Subnet list for guest cluster k8s public API whitelisting.")
