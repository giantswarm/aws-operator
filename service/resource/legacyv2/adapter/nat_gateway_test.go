...
func TestAdapterNATGatewayPrivateRouteTableID(t *testing.T) {
	testCases := []struct {
		description                 string
		customObject                v1alpha1.AWSConfig
		expectedPrivateRouteTableID string
	}{
		{
			description: "basic matching, all fields present",
			customObject: v1alpha1.AWSConfig{
			...
			expectedPrivateRouteTableID: "rtb-12345",
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.description, func(t *testing.T) {
			clients.EC2 = &EC2ClientMock{
				routeTableID: tc.expectedPrivateRouteTableID,
			}
			err := a.getNatGateway(tc.customObject, clients)
			if err != nil {
				t.Errorf("unexpected error %v", err)
			...
			if a.PrivateRouteTableID != tc.expectedPrivateRouteTableID {
				t.Errorf("unexpected PrivateRouteTableID, got %q, want %q", a.PrivateRouteTableID, tc.expectedPrivateRouteTableID)
			}
		})
	}
}
