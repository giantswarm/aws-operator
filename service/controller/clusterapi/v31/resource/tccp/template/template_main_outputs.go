...
{{- define "outputs" -}}
  ...
  {{- end }}
  IngressInsecureTargetGroupsIDs:
    Value: !Ref {{ .Guest.Outputs.IngressInsecureTargetGroupResourceName }}
  IngressSecureTargetGroupsIDs:
    Value: !Ref {{ .Guest.Outputs.IngressSecureTargetGroupResourceName }}
  MasterImageID:
