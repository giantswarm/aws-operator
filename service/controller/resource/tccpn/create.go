...
func (r *Resource) EnsureCreated(ctx context.Context, obj interface{}) error {
...

	{
		...
		if IsNotExists(err) {
			...
			r.event.Emit(ctx, &cr, "CFCreate", fmt.Sprintf("The tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusCreateInProgress))
			r.logger.LogCtx(ctx, "level", "debug", "message", "canceling resource")
			return nil
			...
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("the tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusRollbackInProgress))
			r.event.Emit(ctx, &cr, "CFRollback", fmt.Sprintf("The tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusRollbackInProgress))
			r.logger.LogCtx(ctx, "level", "debug", "message", "canceling resource")
			return nil
		} else if *o.Stacks[0].StackStatus == cloudformation.StackStatusUpdateRollbackInProgress {
			r.logger.LogCtx(ctx, "level", "debug", "message", fmt.Sprintf("the tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusRollbackInProgress))
			r.event.Emit(ctx, &cr, "CFUpdateRollback", fmt.Sprintf("The tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusUpdateRollbackInProgress))
			r.logger.LogCtx(ctx, "level", "debug", "message", "canceling resource")
			return nil
			...
			r.event.Emit(ctx, &cr, "CFRollbackCompleted", fmt.Sprintf("The tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusRollbackComplete))
		} else if *o.Stacks[0].StackStatus == cloudformation.StackStatusUpdateRollbackComplete {
			r.event.Emit(ctx, &cr, "CFUpdateRollbackCompleted", fmt.Sprintf("The tenant cluster's control plane nodes cloud formation stack has stack status %#q", cloudformation.StackStatusUpdateRollbackComplete))
		}
