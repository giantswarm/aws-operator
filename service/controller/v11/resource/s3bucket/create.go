...
func (r *Resource) ApplyCreateChange(ctx context.Context, obj, createChange interface{}) error {
	...
	for _, bucketInput := range createBucketsState {
		...
		_, err = awsClients.S3.CreateBucket(&s3.CreateBucketInput{
		...
		_, err = awsClients.S3.PutBucketTagging(&s3.PutBucketTaggingInput{
			Bucket: aws.String(bucketInput.Name),
			Tagging: &s3.Tagging{
		...
		if bucketInput.IsLoggingBucket {
			_, err = awsClients.S3.PutBucketAcl(&s3.PutBucketAclInput{
			...
			_, err = awsClients.S3.PutBucketLifecycleConfiguration(&s3.PutBucketLifecycleConfigurationInput{
				Bucket: aws.String(key.TargetLogBucketName(customObject)),
		...
		if bucketInput.LoggingEnabled {
			_, err = awsClients.S3.PutBucketLogging(&s3.PutBucketLoggingInput{
				Bucket: aws.String(bucketInput.Name),
