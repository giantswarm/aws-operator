...
func (r *Resource) shouldScale(ctx context.Context, customObject v1alpha1.AWSConfig, currentState, desiredState StackState) (bool, error) {
	...
	if !cc.Status.TenantCluster.TCCP.ASG.IsEmpty() && cc.Status.TenantCluster.TCCP.ASG.MaxSize != key.ScalingMax(customObject) {
		r.logger.LogCtx(ctx, "level", "debug", "message", "scaling due to scaling.max: cc.Status.TenantCluster.TCCP.ASG.MaxSize %d differs from key.ScalingMax(customObject) %d", cc.Status.TenantCluster.TCCP.ASG.MaxSize, key.ScalingMax(customObject))
	...
	if !cc.Status.TenantCluster.TCCP.ASG.IsEmpty() && cc.Status.TenantCluster.TCCP.ASG.MinSize != key.ScalingMin(customObject) {
		r.logger.LogCtx(ctx, "level", "debug", "message", "scaling due to scaling.min: cc.Status.TenantCluster.TCCP.ASG.MinSize %d differs from key.ScalingMin(customObject): %d", cc.Status.TenantCluster.TCCP.ASG.MinSize, key.ScalingMin(customObject))
		if currentState.MasterInstanceType != desiredState.MasterInstanceType {
			r.logger.LogCtx(ctx, "level", "warning", "message", "master instance type will be changed while scaling: current type is %q but desired type is %q", currentState.MasterInstanceType, desiredState.MasterInstanceType)
		}
		return true, nil
