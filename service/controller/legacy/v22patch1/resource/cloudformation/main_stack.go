...
import (
	...
	"github.com/giantswarm/aws-operator/service/controller/legacy/v22patch1/templates"
...
func (r *Resource) getMainGuestTemplateBody(ctx context.Context, customObject v1alpha1.AWSConfig, stackState StackState) (string, error) {
	...
	cfg := adapter.Config{
		...
		StackState: adapter.StackState{
			...
			WorkerDesired:            sc.Status.TenantCluster.TCCP.ASG.DesiredCapacity,
			...
			WorkerMax:                sc.Status.TenantCluster.TCCP.ASG.MaxSize,
			WorkerMin:                sc.Status.TenantCluster.TCCP.ASG.MinSize,
