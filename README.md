...
The [list of AMIs](https://github.com/giantswarm/aws-operator/blob/legacy/service/controller/key/amis.go) is generated by parsing https://stable.release.flatcar-linux.net/amd64-usr/
into a JSON structure, serializing the JSON into a string, and then generating a Go source
file which holds the structure as a multi-line JSON string.

To get information about a new release into `amis.go`,
the file can be regenerated using `go generate ./service/controller/key/generate/generate_amis.go`.
